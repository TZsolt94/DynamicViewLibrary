...
class DynamicViewListHolder(private val act: Activity, var visibleButtonNumber : Int, bArray : Array<Button>){
    private var dvList = this.createDynViews(act, bArray)
    ...
    private fun createDynViews(act : Activity, bArray : Array<Button>) : List<DynamicView>{
    ...
    fun createDynamicView(constraintLayoutID: Int, dynamicButtonReSize : Boolean, marginTop : Int = 0, marginButton : Int = 0, topViewID : Int = constraintLayoutID) {
        ...
        for (i in 0 until dvList.size-1){
            if(dynamicButtonReSize) {
                dvList[i].button.width = (width / (i + quotient)).toInt() + width / 4
            }
            constraintSet.connect(
